...
def schedule_from_config(config):
    if config.get("type") == "linear": ## default
        ...
        raise ValueError(
            "Unsupported schedule type: {}".format(config.get("type")))
...
def evaluate(policy_model, data, step, descriptor, tb_writer, log_dir, k=5):
    ...
    ## subsequences = [data[0:30], data[30:60] ... ]
...
def measure_cache_hit_rate(
        memtrace_path, cache_config, eviction_model, model_prob_schedule, ## schedules.ConstantSchedule
    ...
    line_size = cache_config.get("cache_line_size") ## def:64
    with memtrace.MemoryTrace(memtrace_path, cache_line_size=line_size) as trace: ## 用with會先執行 __enter__ 用完執行 __exit__
        ...
        with tqdm.tqdm(desc=desc) as pbar:
            while not trace.done():
                ...
                cache.hit_rate_statistic.reset() ## reset 次數
                ...
                with evict_trace.EvictionTrace(eviction_trace_path, False) as etrace:
                    ...
                    while len(data) < max_examples and not trace.done():
                        ...
                        pcs.add(pc)
                        cache.read(pc, address, [add_to_data]) ## third para ?? observer
...
def main(_):
    ...
    tf.disable_eager_execution() ## tf 2 使用 tf1 api
    ...
    logging.info('dagger_schedule value : %s', dagger_schedule.value(1)) ## (add)
    ...
    logging.info('schedules step test: %s', schedules.ConstantSchedule(0).value(get_step)) ## (add)
    ...
    # cn =0
    # for valid_data, hitr in (oracle_valid_data):
    #     for batch_num, batch in enumerate(utils.as_batches([valid_data], 32, 80)):
    #         cn +=len(batch)
    #         # logging.info('qqq%s:\t %s', len(batch), cn)
