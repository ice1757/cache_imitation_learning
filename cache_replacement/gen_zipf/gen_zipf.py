...
if __name__ == "__main__":
    ...
    num_files = int(sys.argv[2])    ## num_resources  幾種address
    num_samples = int(sys.argv[3])  ## num_samples    要access幾次
    ...
    for i in range(num_progs):
        ...
        ## imitation trace
        # print([hex(i) for i in requests])
        requests = [hex(i) for i in requests]
        tmp = pd.DataFrame({'pc':hex(0), 'address': requests})
